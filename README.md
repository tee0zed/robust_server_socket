...
RobustServerSocket.configure do |c|
  ...
  # ВАЖНО: Храните в переменных окружения, НЕ коммитьте в git!
  c.private_key = ENV['ROBUST_SERVER_PRIVATE_KEY']
  
  # Время жизни токена в секундах
  # Рекомендация: 1-3 сек для production (время от запроса клиента до сервера)
  c.token_expiration_time = 3
  
  # Список разрешённых сервисов (whitelist)
  # Должен совпадать с именами в keychain клиента
  c.allowed_services = %w[core payments notifications]
  
  # Redis для хранения использованных токенов
  c.redis_url = ENV.fetch('REDIS_URL', 'redis://localhost:6379/0')
  c.redis_pass = ENV['REDIS_PASSWORD'] # может быть nil для локальной разработки
  
  # ОПЦИОНАЛЬНЫЕ ПАРАМЕТРЫ: Rate Limiting
...
# Быстрая валидация с исключениями (рекомендуется)
begin
...
RobustServerSocket.configure do |c|
  ...
  # Для микросервисов с низкой нагрузкой
...
# НЕ игнорируйте ошибки Redis (rate limiter уже fail-open, но логкируйте их)
```
